...
## Web UI(stemlab-web)の技術メモ

- 構成: FastAPI(127.0.0.1 固定)+単一 HTML。分離はサブプロセスで既存 CLI を実行、
  ジョブ状態は out/web/jobs/ の JSON(DB レス)。詳細は .claude/plans/stemlab-web-plan.md
- 敵対的レビューで修正済み: ポーリングの fetch 失敗時再スケジュール /
  /packages の露出面遮断(web/・.cache/)/ CLI 製フォルダとのタイトル衝突 /
  TrustedHost(DNS rebinding)/ **サーバー再起動時の孤児サブプロセス刈り取り**
  (PID サイドカー+コマンドライン照合。試用中に二重実行として実際に発生した)
- 受容済みの既知事項(low): ジョブ属性のロック外読みで一瞬の不整合スナップショット
  があり得る(次のポーリングで自己修復)/ 完了後にパッケージフォルダを手動削除
  するとリンクが 404(ジョブは完了表示のまま)/ 同一内容を拡張子違いで上げると
  uploads に孤児ファイルが 1 つ残る
- アップロード上限 500MB(mp4/mov の動画コンテナ対応のため。音声トラックのみ使用)
