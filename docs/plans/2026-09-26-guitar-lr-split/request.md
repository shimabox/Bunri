# 実装依頼: ギター練習パッケージへの L/R 定位分割トラック追加

## 背景

多くのロック/ポップスでは複数のギターが左右に振り分けられている。現状の練習パッケージは「ギターのみ」stem が 1 本で、左右のパートを聴き分けられない。分離済みギター stem を定位(ステレオ角)で 2 分割し、「L のみ / R のみ」の 2 トラックを練習パッケージとプレイヤーに追加する。分割方式・閾値・名前・UI の挙動は確定済みの仕様(下記「仕様」の方針 9 の判定ロジックと各既定値)で、変えない。

既存パッケージについては、元の音源を用意しなくても、キャッシュに残っている分離済み stem から L/R をまとめて追加できるコマンド `bunri lr-split` を設ける。Web で同じ曲を再アップロードしても完了済みジョブが再利用されて build_package が走らないため、この手段が必要になる。

確定済みの仕様(変更しない):

- scipy を直接依存に加えず、STFT/ISTFT は torch で実装する。
- L/R の有無を身元ファイル `.bunri-package.json` の target 項目のフィールド `pan_split` に記録する。schema_version は 1 のまま。
- L/R は Bunri Pocket には送らない。
- L/R は Web のダウンロード欄に出す。
- 曲一覧で「L/R に分かれていない曲」の注記は出さない。
- 既存パッケージ用に新コマンド `bunri lr-split` を設ける。内部名(モジュール `src/bunri/pan_split.py`・キャッシュ段 `pan_split:<target>`・身元ファイルのフィールド `pan_split`・`TargetSpec.pan_split`・関数 `add_pan_split` / `set_target_pan_split`・ロックファイル `out/.cache/pan_split.lock`)は `pan_split` のままとする。
...
- 承認済み計画: docs/plans/2026-09-26-guitar-lr-split/plan.md
- 承認済み計画の SHA-256: 75559c56bd880ac1afc717d369dbe33d7002807576b4de5a3591294fe9f46d87
- 担当: claude-opus-5-5 / high(通常段)。信号処理の移植(scipy から torch)、パッケージ生成・メタデータ・プレイヤー・Web の横断変更、既存パッケージ向けの新コマンドを含むため
...
1. `src/bunri/registry.py`: `TargetSpec` に `pan_split: bool` を追加する(guitar のみ True、他は False)。docstring を更新する。
2. `src/bunri/pan_split.py` を新設する: torch による STFT/ISTFT(`pad_mode="constant"`、`center=True`、`length=n`)、純 numpy の `decide`(方針 9 の 1〜8。山は正の高さのみ、同値は最小添字)、ファイル I/O を担う `split_stem`、`PanSplitDecision`(json との往復を含む)。定数(n_fft・hop・bins・smooth・min_peak_distance・softness・min_share)をモジュール定数にする。仕様の方針 7・8・9 に従う。
3. `src/bunri/package_metadata.py`: `TargetMetadata.pan_split: str | None = None` を追加し、`_validate` で検証、`_payload` で書き戻す(None は省略)。`complete_target` に `pan_split` 引数を足し、`set_target_pan_split` と `TargetNotFoundError(ValueError)` を追加する(方針 2)。`_locked_update` で `begin_target` / `complete_target` / `set_target_pan_split` を `out/.cache/metadata.lock`(timeout 10 秒)の下に置く(方針 15)。タスク 5 に依存。
4. `src/bunri/cache.py`: `stage_completed(cache_dir, stage_name, outputs)` を追加する(方針 13 の手順 6)。
5. `src/bunri/lock.py` を新設して `ProcessLock`(`timeout` と `busy_error` 付き)と `ProcessLockBusy` を置き、`src/bunri/pocket/lock.py` の `SyncLock` をこれへの委譲に変える。公開 API・文面・ロック位置は変えない(方針 15)。
6. `src/bunri/package.py`: `_pan_split_step` / `_export_pan_split` / `_player_refs` / `_write_player` に切り出し、build_package をこれらで書き直す。キャッシュ段 `pan_split:<target>` の追加、`"single"` 時の旧 L/R 除去、`complete_target` への `pan_split` の受け渡しを行う(方針 5・6・14)。タスク 1〜3 に依存。
7. `src/bunri/package.py`: `add_pan_split` と `PanSplitOutcome` を追加する(方針 13 の前提確認、方針 14 の書き込み順、`TargetNotFoundError` から `failed` への写し)。タスク 3・4・6 に依存。
8. `src/bunri/lr_split_cli.py` を新設し(引数、ロック、進捗、集計、終了コード)、`src/bunri/cli.py` の `dispatch()` に `lr-split` の分岐を追加する(方針 12・15・16)。タスク 5・7 に依存。
9. `src/bunri/player.py` / `src/bunri/templates/player.html.j2`: 引数追加、ボタン・理由文・注意書き・キー 4/5・ヘルプを実装する(方針 10)。
10. `src/bunri/local_package.py`: `_metadata_issues` の検証、`TargetArtifactInspection` に `pan_split` / `left_files` / `right_files` を追加し、`inspect_package_artifacts` で L/R を検査する(`complete_formats`・`issues` は不変。方針 5)。タスク 3 に依存。
11. `src/bunri/web/app.py`: `_serialize_song` のダウンロード欄に L/R 行を追加する(方針 3)。Web 画面側(テンプレート/JS)が downloads 配列を汎用に描画しているか確認し、トラック固定ならその修正も行う。タスク 10 に依存。
...
13. 統合テスト `tests/test_package.py`: 既存フェイク(400 サンプル、両チャンネル同値)で例外なく L/R が作られず sidecar が `"single"`、プレイヤーに理由文。定位を振った stem(2 秒、8 kHz、L に 220 Hz、R に 440 Hz)を書くフェイクで `.left/.right` の wav/mp3・sidecar `"left_right"`・プレイヤー参照、`mp3=False` で wav のみ、再実行で pan_split 段が cached、フィールド無し sidecar の再実行で L/R が追加されること。既存テストの `[cache_dir] = (out_dir / ".cache").iterdir()` を `.cache` 直下のディレクトリだけを選ぶ形に修正する。身元ファイルの target 項目を辞書の完全一致で比較している既存テスト(`sidecar["targets"] == [...]` の箇所など)は、guitar の項目に判定結果の `"pan_split"` を加える更新だけを行う(他の target の項目と他のキーは変えない)(方針 15)。
14. CLI 統合テスト `tests/test_lr_split_cli.py`: 方針 17 の既存パッケージ化ヘルパーを用いて、L/R 追加・single 記録・キャッシュ欠如・stem 不一致・legacy・invalid・記録済みスキップと `--force`・`--all` の集計と終了コード・引数の排他・非対応 target・開始時に guitar の項目が無い(他 target だけ、または再生成中を模擬)ときのスキップと終了コード 0・方針 15 の再分離との交差(開始後の項目消失で失敗と終了コード 1)・二重起動でロック busy・dispatch の分岐。
15. `tests/test_package_metadata.py`・`tests/test_local_package.py`・`tests/test_cache.py`・`tests/test_lock.py`(新設可): フィールドの往復、不正値の拒否、旧形式の受理、`set_target_pan_split` の動作と target 不在時の `TargetNotFoundError`、方針 15 の sidecar 交差実行テスト(両方向)、ロックファイルが `out/.cache/metadata.lock` に作られること、`ProcessLock` の `timeout` の挙動、`stage_completed`、`inspect_package_artifacts` の L/R 検査と `complete_formats` 不変。
16. `tests/test_player_html.py`: 構造テスト(ボタン有無・disabled・理由文・キーのヘルプ)と Playwright(5 トラックで `switchTrack("left")` が切り替わり drift が小さい、single 時は `tracks.left.available` が false でボタンが disabled、キー 4/5)。
17. `tests/test_web_api.py`・`tests/test_web_jobs.py`: L/R ありパッケージで downloads に 4 行、single で 2 行、status/missing_files が L/R に左右されないこと。`add_pan_split` 実行後の曲一覧に L/R が現れること。
18. `tests/test_pocket_local.py`・`tests/test_pocket_sync.py`: L/R ファイルと `pan_split` フィールドがあっても preflight の資産が 3 つで manifest が変わらないこと。`SyncLock` の既存テストが無変更で通ること。
19. README: 「できあがるファイル」表に 2 行追加、ツリー更新、注記、「既存パッケージに L/R を追加する」小節(`bunri lr-split`。方針 18)。CHANGELOG があれば追記する。
20. 検証: `uv run pytest -q -n auto` と Makefile の lint 系ターゲット(存在する場合)を、下記「検証と報告」の方法で実行・記録する。CI(.github/workflows/ci.yml)は既存 workflow に委ねる。
21. commit: 変更対象のファイルを明示して staging し(計画書 2 ファイルを含む)、作業ブランチ plan/2026-09-26-guitar-lr-split へ commit する。commit 後の HEAD で検証を再実行・記録する(未 commit の内容を含めた検証記録は再利用されないため)。

独立実装レビューは采配役が別途行う。実装担当は起動しない。

## 完了条件

各基準の先頭に検証経路(関数単位 / build_package 経由 / CLI 経由 / Web API / Playwright)を示す。

- [ ] 関数単位 `decide`(有限長 STFT): L に 220 Hz、R に 440 Hz(同振幅)、2 秒、8 kHz のステレオを方針 7 の STFT に通した角度・energy で `"left_right"`、境界の区分が 5 以上 39 以下(実測 20)、左側の音量比が 0.45〜0.55(実測 約 0.500)。第 1 の山の添字と境界の厳密な位置は固定しない(実測では平滑化後の全 45 区分が正で、第 1 の山は 43)
- [ ] 関数単位 `decide`(有限長 STFT): 同一信号を両チャンネル(中央)で `"single"`(全点が区分 22、正の区分は 21〜23 だけで候補が無い)。L と同じ波形を R に振幅 0.1 で置いた合成音で `"single"`(全点が区分 2、正の区分は 1〜3 だけ)。無音・モノラル・サンプル数 0 で `"single"` かつ例外なし
- [ ] 関数単位 `decide`(理想化: 角度・energy の配列を直接渡す): 角度 0 と π/2 に同じ総重みで `"left_right"`、谷は 2(第 1 の山 0 に隣接する最初のゼロ区分)、左側比率 0.40〜0.50(約 0.45)。区分 22 の中心角だけに重みで `"single"`。区分 10 と 30 の中心角に同じ重みで `"left_right"` かつ谷は 12(11 より大きく 29 より小さい)。区分 0 と 8 の中心角に同じ重みで谷が 2(最初の最小添字)。区分 9・10・11 の中心角に 0.33 ずつと区分 30 の中心角に 0.01 で、山は 2 つ・谷は 13 だが左側比率が約 0.91 なので閾値 0.10 により `"single"`。同じ配置で区分 30 の重みを 1.0 にすると左側比率 約 0.457(第 1 の山は 29、谷は 13)で `"left_right"`
- [ ] 関数単位 `split_stem`: `left + right` と元 stem の RMS 相対誤差が 1e-3 未満(PCM16 量子化を含む)。出力は元と同じサンプル数・サンプルレート・2 チャンネル。長さ 400・2048・2049・4095・4096・4097・16000 のすべてで例外なく同じ結果
- [ ] 関数単位 `split_stem`: 400 サンプルの定位を振った stem(L に 220 Hz、R に 440 Hz、8 kHz)で `"left_right"` になり、L/R が書かれ、`left + right` が元 stem と一致する。境界の位置と比率は固定しない
...
- [ ] `uv run pytest -q -n auto` が全件通過。Makefile の lint 系ターゲットが通過(存在する場合)。CI の既存 workflow(.github/workflows/ci.yml)は PR 作成後に実行されるため本依頼の範囲外で、緑になることは PR 作成時に確認する
- [ ] `pyproject.toml` の dependencies に scipy を追加していない。`pan_split.py` の `torch.stft` 呼び出しが `pad_mode="constant"` を明示している
- [ ] 作業ブランチへ commit 済み。変更対象を明示して staging し、計画書 2 ファイル(docs/plans/2026-09-26-guitar-lr-split/plan.md と docs/plans/2026-09-26-guitar-lr-split/request.md)を含める

## 計画書の commit 方針

レビュー・privacy 検査済みの共有可能な計画書を含める:

- docs/plans/2026-09-26-guitar-lr-split/plan.md
- docs/plans/2026-09-26-guitar-lr-split/request.md
...
- 担当が判断してよい範囲:
  - 変数名・private 関数の内部構成・テストヘルパーの置き場所などの実装詳細(仕様に名前が定まっているものは変えない)。
  - 未提示ファイル: `src/bunri/separate.py`(stem の wav サブタイプ、stem の配置方法)、Web 画面のテンプレート/JS(downloads の描画方式)、`web/jobs.py`(実行中ジョブの列挙 API の有無、scan_local_packages の呼び出し頻度)、`tests/test_cli.py`(dispatch のテスト方式)、`tests/test_pocket_sync.py`(SyncLock のテスト範囲)、Makefile の lint ターゲット、CHANGELOG の有無。実装担当が参照して方針に沿って判断する。stem のサブタイプが PCM16 でなければ L/R も stem に合わせる。
  - 実行中 Web ジョブの検出: `web/jobs.py` に digest ごとの queued/running ジョブを列挙する安価な既存関数があれば、そのパッケージを「Web で分離ジョブ実行中」としてスキップに回す。無ければ検出は作らず、README の案内と方針 15 のロックによる直列化に委ねる。新しいジョブ記録読み取りは実装しない。既存関数の利用だけ許可する。
  - `_download_files(job)`: ジョブ記録経由の直列化に L/R を出すには sidecar 読み取りが要る。Web 画面が曲一覧(artifact 経路)だけで downloads を描画していれば変更しない。job 経路も使うなら sidecar を読んで追加する。
  - キャッシュ識別ファイルが無いキャッシュ: `ensure_input_identity` が新規作成する(build_package と同じ挙動)。そのままとする。
  - stem の整合確認のコスト: 5 分の stem(約 50 MB)を 2 本 sha256 し、`--all` ではパッケージ数分かかる。許容とし、サイズ不一致で先に打ち切る。
  - ダウングレード時のフィールド消失: 旧版 Bunri が別 target を追加ビルドすると `pan_split` が落ちる。既知の制限として README には書かず、コードコメントに残す(新コマンドを再実行すれば復元できる)。
  - 長尺曲のメモリ: 20 分超の入力で 4 GB 前後に達し得る。チャンク処理は入れず、float32 維持と逐次解放を守る。
  - ラベルの空白: 「L のみ」の空白がダウンロードファイル名に入る(`{title}_L のみ.mp3`)。既存規則のままとし、問題があればラベルからの空白除去のみ判断してよい。
  - 既存 Web テストの期待値: downloads の行数を固定しているテストがあれば行数を更新してよい。
  - `.cache` 直下のロックファイル: `metadata.lock` と `pan_split.lock` を `out/.cache` 直下に置く。`.cache` 直下を列挙してキャッシュ鍵ディレクトリとして扱う処理があれば通常ファイルを無視する必要がある。実装担当が列挙処理を確認し、通常ファイルを無視していなければ直す。既存テストの `[cache_dir] = (out_dir / ".cache").iterdir()` は複数要素になるためディレクトリだけ選ぶ形に直す(方針 15)。テスト修正は同種の箇所に限る。`.cache` を手動で消すと保持中のロックが無効になるが、手動削除中の実行は対象外とする。
  - 有限長 STFT の合成音の実測値: 220/440 Hz 同振幅の実測(第 1 の山 43、谷 20、比率 約 0.500)は基準実装での値で、torch 実装では末尾フレームと浮動小数点で ±1 区分ずれうる。受け入れ条件は範囲指定にした。R を別周波数(440 Hz)で振幅 0.1 にした場合の判定は実測が無いため受け入れ条件に入れない。実装担当が実測して `"single"` なら任意にテストを足してよい。
  - 身元ファイルロックの待ち時間: 10 秒固定。Web の分離ジョブと重なっても区間はミリ秒なので通常は待たない。値の変更は実装担当が判断してよい。
  - 曲削除との交差: delete_song がパッケージを消している最中に sidecar を書き戻すと、削除側が「変更された」で失敗するか sidecar が消える。build_package と同じ既存のリスクであり、本依頼では扱わない。ロックの対象にしない。
...
- 止まって報告する条件:
  - scipy など新しい依存の追加が必要になった場合(`pyproject.toml` の dependencies は変えない)。
  - torch 実装で合成音テストの判定が方針 9 の契約と一致しない場合。また、基準実装で確認した実曲での判定と torch 実装の判定に差があると判明した場合(端処理は scipy の既定 boundary='zeros' と同じ constant にそろえたが、scipy の `padded=True` による末尾の半端フレームと浮動小数点の差で境界が ±1 区分ずれうる。代替案は scipy を直接依存に追加することだが、実装担当は採用しない)。
  - 有限長 STFT の合成音(220/440 Hz 同振幅)の境界・比率が完了条件の範囲を外れた場合。
  - 平滑化後にゼロ区分がある実曲で基準実装と判定に差が出た場合(「高さ 0 を山にしない」規則は、ある角度帯にエネルギーが厳密に 0 の stem でだけ基準実装と結果が変わりうる)。
  - 身元ファイルロックの待ち時間の超過が観測された場合。
  - 身元ファイルの schema_version 変更が必要になった場合。
  - Pocket の送信内容(preflight の資産・manifest)が変わる場合。
  - 承認範囲外のファイルを変える必要が出た場合(CI 設定、分離モデル関連 `src/bunri/separate.py` の挙動変更、`pyproject.toml` の version など)。
  - 実行中 Web ジョブの検出に、既存関数ではない新しいジョブ記録読み取りが必要になった場合。
  - キャッシュ識別ファイルの作成を避けるべき(読み取り専用の照合関数が必要)と判断した場合。
  - stem の整合確認が遅すぎると判明した場合。
  - 長尺曲でメモリの問題が出た場合。
  - 既存 Web テストで行数以外の期待値を変える必要が出た場合。
  - 仕様の方針 1〜18 や受け入れ基準と矛盾する実装が必要になった場合、その他の実質的なスコープ変更。
- 公開経路と許可範囲: 作業ブランチ plan/2026-09-26-guitar-lr-split への commit までを許可する。push・PR 作成・main への統合・リリース(バージョン更新)は本依頼の範囲外で、別途承認を得てから行う。CI は PR 作成後に .github/workflows/ci.yml で実行される。

## 実行上の制約

- この実装担当は commit までとし、push・PR 作成・公開を行わない
- commit subject は日本語の 1 行とする(このリポジトリの慣例)。変更内容と理由を簡潔に書く
- `pyproject.toml` の dependencies に scipy を加えない
- `pyproject.toml` の version を変えない
- サブエージェントを生成しない
...
検証コマンド(`uv run pytest -q -n auto`、Makefile の lint 系ターゲット)は、次のラッパーを通して起動する。ラッパーは command(実際の引数)、cwd、対象範囲、開始・終了時刻、前後の HEAD と tracked / index 状態、終了コード、標準出力・標準エラーを、呼出しごとの private な証拠ファイルへ書き出す。担当が後から文章やファイルを手書きして実行結果の代わりにしない。

証拠の保存先は、采配役が実行時添付情報で指定するリポジトリ外の private ディレクトリ(`verify_evidence_parent`)とする。采配役は事前に `verify_worktree`(実装 worktree)、`verify_scope`(検証範囲の説明)、`verify_command`(コマンドと引数の配列。例: `uv run pytest -q -n auto`)、`verify_evidence_parent` を用意する。各実行がその配下に新しい証拠ディレクトリを作り、既存の証拠を上書き・再利用しない。呼出し元が zsh でも Bash を明示して起動する。検証コマンドの stdin は `/dev/null` に固定し、入力を必要としない非対話コマンドだけを対象にする。文字列の `eval` は使わない。
...
報告には、各実行で出力された `evidence_dir=` の場所を含める。

再利用条件: 采配役は証拠ファイルを直接読み、実行したコマンド・cwd・範囲が必要な検証に一致し、実行・ログ保存の終了コード(`exit-code.txt`・`log-exit-code.txt`)が両方 0 で、出力が途中で欠けていないことを確認する。実行前後の HEAD が今回の対象 HEAD と一致し、tracked / index の差分が双方とも空であることも照合する。同じ HEAD でも未 commit の内容を含めて実行した記録は再利用しないため、実装担当は commit 後の HEAD で検証を実行・記録する。untracked / ignored は予定の計画書・検証生成物等かを確認し、検証入力への影響が不明なものがあれば再利用しない。必要な実行環境や検証範囲が変わった場合、以前の別作業の古い記録、対応する実行を確認できない記録、項目が不足した記録も再利用しない。担当の自己申告を機械的な証拠と混同しない。

保存不可時のフォールバック: 担当の sandbox からリポジトリ外の private 領域へ書けない場合は、証拠を捏造せず、テスト結果の要約だけを報告したうえで「采配役側で対象 worktree の同じ検証コマンドを実行・記録する」必要があることを報告する。生ログをリポジトリへ保存したり、sandbox を緩和したりして解消しない。采配役側でも実行・記録できなければ未検証として理由を報告する。CI の結果は PR 作成後に対象 SHA・必須 check・実行結果を照合して確認するもので、本依頼の範囲外とする。
