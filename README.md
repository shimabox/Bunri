...
- 曲を開いて「この曲を削除」を選ぶと、その曲の練習パッケージ、全ジョブ履歴とログ、ほかの曲と共有していないアップロード元とキャッシュをまとめて削除できます。Pocket 接続中は、確認画面で「Bunri Pocket の棚からも削除する」を明示的に選べます。この場合は棚の削除成功を確認してからローカルデータを削除します。待機中または処理中のジョブを含む曲は削除できません。削除は取り消せないため、確認画面の内容を確認してから実行してください
...
同じ出力先で Bunri Pocket に接続済みの場合、Web UI に「Bunri Pocket 連携中」と表示されます。曲ごとの `棚にある` / `未同期` / `差分あり` の確認と個別アップロード、出力先の全パッケージを対象にした全曲アップロードを利用できます。全曲アップロードには、Web の曲一覧にない CLI 生成パッケージも含まれます。棚にだけ残っている曲は、ローカル曲と分けた読み取り専用の「棚にのみある曲」一覧に表示されます。棚のみの曲を削除する場合は CLI の `--select` または `--song-id` を使います。接続先 URL や upload token は画面や API には表示されません。
